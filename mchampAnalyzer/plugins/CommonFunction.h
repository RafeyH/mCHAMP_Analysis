...
reco::DeDxData computedEdx(const int& run_number,
  ...
  for (unsigned int h = 0; h < dedxHits->size(); h++) {
    ...
    if (detid.subdetId() >= 3) {  //for strip only
      ...
      if (crossTalkInvAlgo == 1){
        ...
        //for (auto temp : amplitudes) cout<<temp<<" ";
		//cout<<endl<<"Amplitudes after: ";
	...
	//cout<<"Cluster charge: "<<ClusterCharge<<endl;
  ...
  if (dropLowerDeDxValue > 0) {
	...
	//cout<<"Charge over path length: ";
	//for(auto temp: vect) cout<<temp<<" ";
	//cout<<endl;
